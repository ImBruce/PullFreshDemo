...
@property (nonatomic, strong) SVPullToRefreshArrow *freshArrow;

@property (nonatomic, strong) LKSLoadingView *loadingIndicatorView;
...
@synthesize freshArrow = _freshArrow;
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        CGFloat leftViewWidth = MAX(self.arrow.bounds.size.width,self.loadingIndicatorView.bounds.size.width);
         ...
         */
        self.arrow.frame = CGRectMake(85.0f,
                                      (self.bounds.size.height - self.arrow.bounds.size.height)/2,
                                      self.arrow.bounds.size.width,
                                      self.arrow.bounds.size.height);
        
        self.loadingIndicatorView.center = self.arrow.center;
    }
}
...
- (SVPullToRefreshArrow *)arrow {
    if(!_freshArrow) {
		_freshArrow = [[SVPullToRefreshArrow alloc]initWithFrame:CGRectMake(85.0f,(self.bounds.size.height - 25.0)/2,25.0,25.0)];
        _freshArrow.backgroundColor = [UIColor clearColor];
		[self addSubview:_freshArrow];
    }
    return _freshArrow;
}

- (LKSLoadingView *)loadingIndicatorView {
    if (!_loadingIndicatorView) {
        _loadingIndicatorView = [[LKSLoadingView alloc] initWithFrame:CGRectMake(0, 0, 50, 50)];
        _loadingIndicatorView.endless = YES;
...
- (void)rotateArrow:(float)degrees hide:(BOOL)hide {
    ...
    [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionAllowUserInteraction animations:^{
        self.arrow.layer.transform = CATransform3DMakeRotation(degrees, 0, 0, 1.0);
...
- (void)drawRect:(CGRect)rect {
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"ic_pulltorefresh_arrow_gold" ofType:@"png"];
    UIImage *img = [UIImage imageWithContentsOfFile:path];
    CGImageRef image = img.CGImage;
    CGContextSaveGState(context);
    CGRect touchRect = CGRectMake((self.bounds.size.width - img.size.width)/2, (self.bounds.size.height - img.size.height)/2, img.size.width, img.size.height);
