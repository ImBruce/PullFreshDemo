...
- (void)layoutSubviews {
    self.loadingIndicatorView.center = CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2);
}
...
- (void)setState:(SVInfiniteScrollingState)newState {
    ...
    if(hasCustomView) {
        ...
        CGRect viewBounds = [self.loadingIndicatorView bounds];
        CGPoint origin = CGPointMake(roundf((self.bounds.size.width-viewBounds.size.width)/2), roundf((self.bounds.size.height-viewBounds.size.height)/2));
        [self.loadingIndicatorView setFrame:CGRectMake(origin.x, origin.y, viewBounds.size.width, viewBounds.size.height)];
